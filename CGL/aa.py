...
class get_money(object):
    ...
    def one_team(start, end):
        ...
        one_team = TeamModel.objects.filter(parent_id=1)
        ...
        for i in two_team:
            ...
            # email = ''
            # Q5 = Q(isleader='1', status='Active', team=i)
            # Q6 = Q(isleader='1', status='Leave', team=i, leaveDate__gt=end)
            # email_obj = UserModel.objects.filter(Q5 | Q6).values('email', 'status')
            # 兼容
            # if name =='Tony Hang':
            #     email_obj = UserModel.objects.filter(name__icontains=name).values('email', 'status')
            # if name =='Catherine Meiji Tantai':
            #     email_obj = UserModel.objects.filter(name__icontains='Catherine MEIJI').values('email', 'status')
            # if email_obj.exists():
            #     email = email_obj[0]['email']
            res_go_out_user = {}
            #     if email_obj[0]['status'] == 'Leave':
            #         res_go_out_user[i.id] = {'id': i.id, 'name': i.name, 'billing': billing_sum, 'invoice': invoice_sum,
            #                                  'send': send_sum, 'leader': name, 'index': index, 'email': email}
            #     else:
